    ...
    <Box className='px-4 py-4 rounded-md' sx={{ bgcolor: 'background.paper', marginTop: '10px' }}>
      ...
      {recenttransaction.map((item, indx) => {
          return (<div className='flex items-center justify-between py-1' key={indx+"recent"}>
            ...
            <div>{item?.type === 'expense' ? <div className='bg-red-200 px-2 py-1 rounded-md'><p className='text-red-500 flex items-center text-sm'>- <LiaRupeeSignSolid />{item?.amount} <IoMdTrendingDown /></p></div> : 
            <div className='bg-green-200 px-2 py-1 rounded-md'><p className='text-green-600 flex items-center text-sm'>+<LiaRupeeSignSolid />{item?.amount} <IoMdTrendingUp /></p></div> }</div>
          </div>)
      })}
